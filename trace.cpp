...
void ray_trace(s_screen &s, std::vector<shape*> &world, std::vector<light*> &lights, string filename) {
...
{	s_screen screen;
	...
	ray eye = {Vector3d (0, 0, 0.5), Vector3d (0, 0, -1+i)};
	gen_screen(eye, &screen);
	sprintf(filename, "animation%d.png", i);
	ray_trace(screen, *w, *l,filename);
	l->pop_back();
	l->push_back(new light(Vector3d(15, 15, 5+i), colour(255, 255, 255)));
